  ...
  const memoizedTrackList = React.useMemo(() => {
    return displayTracks.map((track, idx) => {
      ...
      return (
        <div
          key={`${track.id}-${idx}`}
          onClick={() => { if (!track.missing) playTrack(idx); }}
          title={track.missing ? `このPCにファイルがありません: ${track.fileName}\nファイルをドラッグ&ドロップするか、フォルダを読み込んでください` : undefined}
          className="group flex items-center text-[11px] h-10 px-2 border-b transition-colors shrink-0"
          style={{ 
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-4 flex-1 min-h-0 border p-4 relative" style={{ backgroundColor: 'var(--theme-surface)', borderColor: 'var(--theme-border)' }}>
        ...
        <div className="border flex flex-col shrink-0 relative" style={{ width: sidebarWidth, minWidth: 120, maxWidth: 400, backgroundColor: 'var(--theme-bg)', borderColor: 'var(--theme-border)' }}>
          <div className="border-b px-2 py-1 flex items-center justify-center h-8" style={{ backgroundColor: 'var(--theme-surfaceLighter)', borderColor: 'var(--theme-border)' }}>
             ...
             <div className="flex items-center gap-4">
               ...
               {displayTracks.length > 0 && (
                 <div className="flex items-center gap-3 border-l pl-3" style={{ borderColor: 'var(--theme-border)' }}>
                   <button 
                     onClick={() => {
                        if (selectedTrackIds.size === displayTracks.length) {
                            setSelectedTrackIds(new Set());
                        } else {
                     ...
                     style={{ color: selectedTrackIds.size === displayTracks.length ? 'var(--theme-accent)' : 'var(--theme-textMuted)' }}
                   >
                     {selectedTrackIds.size === displayTracks.length ? <Check size={10} /> : <div className="w-[10px] h-[10px] border rounded-[1px]" style={{ borderColor: 'currentcolor' }}></div>}
                     {selectedTrackIds.size === displayTracks.length ? 'DESELECT ALL' : 'SELECT ALL'}
                   </button>
